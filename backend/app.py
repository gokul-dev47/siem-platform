...
def mock_stats():
    now = datetime.utcnow()
    hours = [(now - timedelta(hours=i)).strftime("%H:00")
...
def mock_logs(limit=50):
    ...
    for i in range(limit):
        ...
        logs.append({"id": hashlib.md5(f"{i}{ts}".encode()).hexdigest()[:12],
                     ...
                     "severity": et[1],
                     "message": et[2],
                     "source_ip": f"{random.randint(1,
                                                    223)}.{random.randint(0,
                                                                          255)}.{random.randint(0,
                                                                                                255)}.{random.randint(1,
                                                                                                                      254)}",
                     "dest_port": random.choice([22,
                                                 80,
                                                 443,
                                                 3306,
                                                 8080,
...
def mock_alerts(limit=20):
    ...
    for i in range(limit):
        ...
        alerts.append({"id": aid,
                       "alert_id": aid,
                       "timestamp": ts.isoformat() + "Z",
                       "severity": sev,
                       "rule_name": random.choice(["SSH_BRUTE_FORCE",
                                                   "SQL_INJECTION",
                                                   "PORT_SCAN",
                                                   "XSS",
                                                   "RCE_ATTEMPT"]),
                       "source_ip": f"{random.randint(1,
                                                      200)}.{random.randint(0,
                                                                            255)}.{random.randint(0,
                                                                                                  255)}.{random.randint(1,
                                                                                                                        254)}",
                       "description": f"Rule triggered {random.randint(5,
                                                                       100)} times in 60s",
                       "status": random.choice(["open",
                                                "investigating",
                                                "closed"]),
                       "count": random.randint(5,
                                               500)})
...
def es_stats():
    ...
    try:
        ...
        atypes = {b["key"]: b["doc_count"]
                  for b in r["aggregations"]["by_type"]["buckets"]}
        ips = [{"ip": b["key"], "count": b["doc_count"], "country": "??"}
               for b in r["aggregations"]["top_ips"]["buckets"]]
        hourly = [{"hour": b["key_as_string"][:16], "count": b["doc_count"]}
                  for b in r["aggregations"]["per_hour"]["buckets"][-24:]]
        return {
            "total_events": total,
            "critical_alerts": sev.get(
                "critical",
                0),
            "blocked_ips": len(ips),
            "active_threats": sev.get(
                "critical",
                0) +
            sev.get(
                "high",
    ...
    try:
        ...
        if query:
            must.append({"multi_match": {"query": query, "fields": [
                        "message", "source_ip", "event_type"]}})
        if severity:
            must.append({"term": {"severity.keyword": severity}})
        r = es.search(index="siem-logs-*",
                      body={"size": limit,
        ...
        return [{"id": h["_id"][:12],
                 ...
                 "message": h["_source"].get("message",
                                             ""),
                 "source_ip": h["_source"].get("source_ip",
                                               "0.0.0.0"),
                 "dest_port": h["_source"].get("dest_port",
                                               0),
                 "protocol": h["_source"].get("protocol",
                                              "TCP"),
                 "raw": h["_source"].get("raw_log",
                                         "")} for h in hits]
    except Exception as e:
        logger.debug(f"ES logs: {e}")
        return mock_logs(limit)
...
def evaluate_rules():
    ...
    for rule in load_rules().get("rules", []):
        if not rule.get("enabled", True):
            continue
        try:
            count = int(r.get(f"siem:counter:{rule['id']}") or 0)
        except Exception:
            continue
        if count >= rule.get("threshold", 5):
            ts = datetime.utcnow().isoformat() + "Z"
            aid = hashlib.md5(
                f"{rule['id']}{time.time()}".encode()).hexdigest()[:10]
            alert = {
                "id": aid,
                "alert_id": aid,
                "timestamp": ts,
            ...
            if SLACK_URL:
                send_slack(alert)
            try:
                r.delete(f"siem:counter:{rule['id']}")
            except Exception:
                pass
...
def save_alert(alert):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """INSERT INTO alerts (alert_id,timestamp,severity,rule_name,description,count,status)
        ...
        req_lib.post(
            SLACK_URL, json={
                "text": f"SIEM ALERT [{
                    ...
                    alert['rule_name']}", "attachments": [
                    {
                        "color": "danger", "fields": [
                            {
                                "title": "Rule", "value": alert["rule_name"], "short": True}, {
                                    "title": "Count", "value": str(
                                        alert["count"]), "short": True}]}]}, timeout=5)
    except Exception:
        pass
...
def broadcast_stats():
    ...
    try:
        ...
        if r:
            try:
                r.setex("siem:stats:latest", 10, json.dumps(stats))
            except Exception:
                pass
        try:
...
def health():
    ...
    try:
        ...
        svcs["elasticsearch"] = {
            "status": "healthy" if r.status_code == 200 else "starting"}
    except Exception:
    ...
    try:
        ...
        if conn:
            ...
            svcs["postgres"] = {"status": "unhealthy"}
    except Exception:
        svcs["postgres"] = {"status": "unhealthy"}
    delta = datetime.utcnow() - START_TIME
    overall = "healthy" if all(
        s["status"] == "healthy" for s in svcs.values()) else "degraded"
...
@app.route("/api/logs")
def api_logs():
    return jsonify({"logs": es_logs(min(int(request.args.get("limit", 100)), 500), request.args.get(
        "q") or None, request.args.get("severity") or None), "total": 100})
...
def api_alerts():
    conn = get_pg()
    if not conn:
        return jsonify({"alerts": mock_alerts(30)})
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            cur.execute(
                f"SELECT * FROM alerts {clause} ORDER BY timestamp DESC LIMIT 100",
                params)
            rows = [dict(r) for r in cur.fetchall()]
            if not rows:
                return jsonify({"alerts": mock_alerts(30)})
            for row in rows:
                row["id"] = row["alert_id"] = row.get(
                    "alert_id", str(row.get("id", "")))
            return jsonify({"alerts": rows})
    except Exception as e:
        logger.debug(f"alerts: {e}")
        return jsonify({"alerts": mock_alerts(30)})
...
def update_alert(aid):
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE alerts SET status=%s WHERE alert_id=%s", (status, aid))
        except Exception as e:
...
scheduler = BackgroundScheduler(
    daemon=True, executors={
        "default": ThreadPoolExecutor(
            max_workers=4)}, job_defaults={
                "coalesce": True, "max_instances": 1, "misfire_grace_time": 15})
